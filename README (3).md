...
- **📊 Real-Time Traffic Chart** — Live rolling line chart (last 20 data points) showing packet volume over time.
...
> **Take your screenshots in this order while the app is running with live traffic.**

| Screen | What to capture | Filename |
|---|---|---|
| **Login Page** | The centered login card on the dark background | `screenshots/login.png` |
| **Dashboard — Stats Bar** | The three stat cards (Status ● running, Total Packets, Total Alerts) with real numbers | `screenshots/dashboard.png` |
| **Traffic Chart** | The line chart showing a spike pattern after running `ping -f` or `nmap` | `screenshots/traffic-chart.png` |
| **IP Geolocation Table** | At least 2–3 rows with city/country/ISP data resolved | `screenshots/geolocation.png` |
| **Alerts Table** | A mix of CRITICAL, HIGH, and MEDIUM severity badges with source IPs and descriptions | `screenshots/alerts.png` |

> **Tip to generate test data fast:** run `sudo nmap -sS <your-own-ip>` and `ping -f localhost` from a second terminal. Within one 5-second analysis cycle you'll have SYN Flood + Port Scan + ICMP Flood alerts populated.
...
- Python 3.10+
- Node.js 18+
- `libpcap` / `npcap` (required by Scapy for raw packet capture)
- Root / Administrator privileges for the backend (packet sniffing requires elevated permissions)
...
```bash
# Packet sniffing requires root on Linux/macOS
...
Open `backend/detection/rules.py` and add a new detector function following the same pattern, then register it in `run_all_rules`:
...
def run_all_rules(buffer: list) -> list:
    ...
    alerts.extend(detect_udp_flood(buffer))   # ← add here
    return alerts
```
...
To capture on a specific interface, change the `interface` argument in `main.py`:
